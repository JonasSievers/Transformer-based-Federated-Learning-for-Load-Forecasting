...
N_CLUSTERS = 9
y = np.loadtxt(cwd+'/data/d04_clients_clustered/d04_clients_clustered_N9.csv', delimiter=',').astype(int)
...
global_LSTM_models, global_CNN_models, global_Transformer_models = createGlobalModelsForClusters(
        windows_dict, INPUT_SHAPE[1], OUT_STEPS[0], NUM_FEATURES[1], 'Federated_LSTM_F7_H12', 'Federated_CNN_F7_H12', 'Federated_Transformer_F7_H12',
...
initiallySaveAllGlobalModelsN9(cwd, global_LSTM_models, global_CNN_models, global_Transformer_models)
...
for idx_com, comm_round in enumerate(range(comms_round)):
    ...
    for idx, cluster in enumerate(windows_dict):
        ...
        #Get global models
        global_LSTM_model, global_CNN_model, global_Transformer_model = loadGlobalModelsN9( 
        ...
        for client in windows_dict[cluster].keys():
            ...
            local_LSTM_model = LSTM_Model().build(
                ...
                LSTM_DROPOUT, OUT_STEPS[0], NUM_FEATURES[1], 'Federated_local_LSTM_F7_H12'
            )
            scaled_weights = compile_fit_set_weights(
                local_LSTM_model, 
                global_LSTM_weights, 
                windows_dict[cluster][client][2], 
                client, 
                client_names, 
                MAX_EPOCHS, 
            ...
            local_CNN_model = CNN_Model().build(
                ...
                CNN_DROPOUT, OUT_STEPS[0], NUM_FEATURES[1],'Federated_local_CNN_F7_H12'
            )    
            scaled_weights = compile_fit_set_weights(
                local_CNN_model, 
                global_CNN_weights, 
                windows_dict[cluster][client][2], #F7H12
                client, 
                client_names, 
                MAX_EPOCHS, 
            )
            ...
            local_Transformer_model = Transformer_Model().build(
                INPUT_SHAPE[1],OUT_STEPS[0],NUM_FEATURES[1],'Federated_local_Transformer_F7_H12'    
            )
            scaled_weights = compile_fit_set_weights(
        ...
        #Save global models
        saveGlobalModelsN9(cwd, global_LSTM_models[idx], global_CNN_models[idx], global_Transformer_models[idx], idx, idx_com)
