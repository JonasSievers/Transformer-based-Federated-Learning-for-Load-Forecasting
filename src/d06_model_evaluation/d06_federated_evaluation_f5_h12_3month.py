...
with open(cwd + '/results/Federated_results_H12_F5_3month_1y.pkl', 'wb') as f:
...
for idx, cluster in enumerate(windows_dict):
    ...
    for i, client in enumerate(windows_dict[cluster].keys()):
        ...
        #LSTM
        model_evaluation_test = loadCompileEvaluateModel(
            path = cwd + f"/data/d05_models/3month/cluster{idx}/Federated_LSTM_F5_H24/FederatedRound19",
        ...
        #CNN
        model_evaluation_test = loadCompileEvaluateModel(
            path = cwd + f"/data/d05_models/3month/cluster{idx}/Federated_CNN_F5_H24/FederatedRound19",
            window = windows_dict[cluster][client][1], 
            MAX_EPOCHS = MAX_EPOCHS
        )
        #Save
        ...
        #Transformer
        model_evaluation_test = loadCompileEvaluateModel(
            path = cwd + f"/data/d05_models/3month/cluster{idx}/Federated_Transformer_F5_H24/FederatedRound19",
            window = windows_dict[cluster][client][1], 
            MAX_EPOCHS = MAX_EPOCHS
        )
        #Save
...
with open(cwd + '/results/Federated_results_H24_F5_3month_1y.pkl', 'wb') as f:
    pickle.dump(final_dict, f)
