...
for idx, cluster in enumerate(windows_dict):
    ...
    for i, client in enumerate(windows_dict[cluster].keys()):
        ...
        #LSTM
        model_evaluation_test = loadCompileEvaluateModelN9(
            path = cwd + f"/data/d05_models/N9/cluster{idx}/Federated_LSTM_F5_H12/FederatedRound19",
        ...
        #CNN
        model_evaluation_test = loadCompileEvaluateModelN9(
            path = cwd + f"/data/d05_models/N9/cluster{idx}/Federated_CNN_F5_H12/FederatedRound19",
            window = windows_dict[cluster][client][0], 
            MAX_EPOCHS = MAX_EPOCHS,
            client = client
        )
        ...
        #Transformer
        model_evaluation_test = loadCompileEvaluateModelN9(
            path = cwd + f"/data/d05_models/N9/cluster{idx}/Federated_Transformer_F5_H12/FederatedRound19",
            window = windows_dict[cluster][client][0], 
            MAX_EPOCHS = MAX_EPOCHS,
            client = client
        )
...
with open(cwd + '/results/Federated_results_H12_F5_N9.pkl', 'wb') as f:
