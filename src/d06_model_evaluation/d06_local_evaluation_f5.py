...
for idx, cluster in enumerate(windows_dict):
    ...
    for i, client in enumerate(windows_dict[cluster].keys()):
        ...
        model_evaluation_test = loadCompileEvaluateModel(
            path = cwd + f"/data/d05_models/cluster{idx}/Federated_LSTM_F5_H12/FederatedRound19",
        ...
        #CNN
        model_evaluation_test = loadCompileEvaluateModel(
            path = cwd + f"/data/d05_models/cluster{idx}/Federated_CNN_F5_H12/FederatedRound19",
            window = windows_dict[cluster][client][0], 
            MAX_EPOCHS = MAX_EPOCHS
        )
        #Save
        ...
        #Transformer
        model_evaluation_test = loadCompileEvaluateModel(
            path = cwd + f"/data/d05_models/cluster{idx}/Federated_Transformer_F5_H12/FederatedRound19",
            window = windows_dict[cluster][client][0], 
            MAX_EPOCHS = MAX_EPOCHS
        )
        #Save
...
#Evaluate Results
forecasts_dict_LSTM_F5_H24 = {k: {} for k in range(N_CLUSTERS)}
forecasts_dict_CNN_F5_H24 = {k: {} for k in range(N_CLUSTERS)}
forecasts_dict_Transformer_F5_H24 = {k: {} for k in range(N_CLUSTERS)}
...
for idx, cluster in enumerate(windows_dict):
    ...
    for i, client in enumerate(windows_dict[cluster].keys()):
        ...
        #LSTM
        model_evaluation_test = loadCompileEvaluateModel(
            path = cwd + f"/data/d05_models/cluster{idx}/Federated_LSTM_F5_H24/FederatedRound19",
            window = windows_dict[cluster][client][1], 
            MAX_EPOCHS = MAX_EPOCHS
        )
        #Save
        forecasts_dict_LSTM_F5_H24[cluster][client] = {
            'MSE':model_evaluation_test[0], 'RMSE':model_evaluation_test[1], 'MAPE':model_evaluation_test[2],
            'MAE':model_evaluation_test[3], 'Time':((timetaken.logs[-1][1]) / (timetaken.logs[-1][0]+1)) 
        }
        print("Saved LSTM")
        ...
        #CNN
        model_evaluation_test = loadCompileEvaluateModel(
            path = cwd + f"/data/d05_models/cluster{idx}/Federated_CNN_F5_H24/FederatedRound19",
            window = windows_dict[cluster][client][1], 
            MAX_EPOCHS = MAX_EPOCHS
        )
        #Save
        forecasts_dict_CNN_F5_H24[cluster][client] = {
            'MSE':model_evaluation_test[0], 'RMSE':model_evaluation_test[1], 'MAPE':model_evaluation_test[2],
            'MAE':model_evaluation_test[3], 'Time':((timetaken.logs[-1][1]) / (timetaken.logs[-1][0]+1)) 
        }    
        print("Saved CNN")
        ...
        #Transformer
        model_evaluation_test = loadCompileEvaluateModel(
            path = cwd + f"/data/d05_models/cluster{idx}/Federated_Transformer_F5_H24/FederatedRound19",
            window = windows_dict[cluster][client][1], 
            MAX_EPOCHS = MAX_EPOCHS
        )
        #Save
        forecasts_dict_Transformer_F5_H24[cluster][client] = {
            'MSE':model_evaluation_test[0], 'RMSE':model_evaluation_test[1], 'MAPE':model_evaluation_test[2],
            'MAE':model_evaluation_test[3], 'Time':((timetaken.logs[-1][1]) / (timetaken.logs[-1][0]+1)) 
        }
        print("Saved Transformer")
...
#Inintialize result dictionary
final_dict = InititalizeResultDictionary(learning_style="Federated")
final_dict['Federated']['LSTM']['H24']['F5'] = forecasts_dict_LSTM_F5_H24
final_dict['Federated']['CNN']['H24']['F5'] = forecasts_dict_CNN_F5_H24
final_dict['Federated']['Transformer']['H24']['F5'] = forecasts_dict_Transformer_F5_H24

with open(cwd + '/results/Federated_results_H24_F5.pkl', 'wb') as f:
    pickle.dump(final_dict, f)