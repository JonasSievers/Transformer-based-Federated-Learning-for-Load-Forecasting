...
for idx, client in enumerate(smart_meter_names):
    ...
    model_evaluation_test = loadCompileEvaluateModel(
        path = cwd + f"/data/d05_models/3month/local/{client}/Local_LSTM_F7_H12/Round100",
    ...
    #CNN
    model_evaluation_test = loadCompileEvaluateModel(
        path = cwd + f"/data/d05_models/3month/local/{client}/Local_CNN_F7_H12/Round100",
        window = windows_dict[client][2], 
        MAX_EPOCHS = MAX_EPOCHS
    )
    #Save
    ...
    #Transformer
    model_evaluation_test = loadCompileEvaluateModel(
        path = cwd + f"/data/d05_models/3month/local/{client}/Local_Transformer_F7_H12/Round100",
        window = windows_dict[client][2], 
        MAX_EPOCHS = MAX_EPOCHS
    )
    #Save
...
with open(cwd + '/results/Local_results_H12_F7_3month.pkl', 'wb') as f:
...
for idx, client in enumerate(smart_meter_names):
    ...
    print("-----------------------", idx, "/32")
            ...
            #LSTM
    model_evaluation_test = loadCompileEvaluateModel(
        path = cwd + f"/data/d05_models/3month/local/{client}/Local_LSTM_F7_H24/Round100",
    ...
    #CNN
    model_evaluation_test = loadCompileEvaluateModel(
        path = cwd + f"/data/d05_models/3month/local/{client}/Local_CNN_F7_H24/Round100",
        window = windows_dict[client][3], 
        MAX_EPOCHS = MAX_EPOCHS
    )
    #Save
    ...
    #Transformer
    model_evaluation_test = loadCompileEvaluateModel(
        path = cwd + f"/data/d05_models/3month/local/{client}/Local_Transformer_F7_H24/Round100",
        window = windows_dict[client][3], 
        MAX_EPOCHS = MAX_EPOCHS
    )
    #Save
...
with open(cwd + '/results/Local_results_H24_F7_3month.pkl', 'wb') as f:
    pickle.dump(final_dict, f)
